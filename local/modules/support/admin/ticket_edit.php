...
<tr>
	<td colspan="2">
		...
		//Проверим, не была ли раньше создана задача с таким титлом:
		$res=CTicket::GetByID($ID);
			    if ($task=$res->Fetch())
				{
				if (CModule::IncludeModule("tasks"))
					{
						...
						$resTask = CTasks::GetList(
							Array(), 
							Array("TITLE"=>"Обработать обращение клиента:  №".$ID." (".$task['TITLE'].")")
						);

						if($arTask = $resTask->GetNext())
						{
							print "Задача для обращения №".$ID." создана <br/><br/>";
							/*print "<pre>";
							print_r($arTask);
							print "</pre>";*/
						}
		...
		if ($_REQUEST['add_task'] =='Создать задачу'){
		...
		/*	print "<pre>";
			...
			print "</pre>";*/
				...
				while ($task=$res->Fetch())
				{
					...
					$rsUser = CUser::GetByLogin($task['RESPONSIBLE_LOGIN']);
					if($arUser = $rsUser->Fetch())
					{
						$respon_id=$arUser['ID'];
					}
					else{
						$respon_id=$USER->GetID();
						...
						$resTask = CTasks::GetList(
							Array(), 
							Array("TITLE"=>"Обработать обращение клиента:  №".$ID." (".$task['TITLE'].")","RESPONSIBLE_ID" =>$respon_id)
						);

						if($arTask = $resTask->GetNext())
						{
							print "Задача для обращения №".$ID." была создана ранее<br/><br/>";
						}
						else{
									
									$dt1=date($DB->DateFormatToPHP(CSite::GetDateFormat("FULL")), time());

									$dt2=ConvertTimeStamp(MakeTimeStamp($dt1,"DD.MM.YYYY HH:MI:SS")+30*3600,"FULL","ru");
						
									$arFields = Array(
										"TITLE" => "Обработать обращение клиента:  №".$ID." (".$task['TITLE'].")",
										"DESCRIPTION" => "Обработать обращение клиента <a href='http://artes-grp.ru/bitrix/admin/ticket_edit.php?ID=".$ID."&lang=ru'>http://artes-grp.ru/bitrix/admin/ticket_edit.php?ID=".$ID."&lang=ru</a>",
										"RESPONSIBLE_ID" => $respon_id,
									...
									$obTask = new CTasks;
									$newID = $obTask->Add($arFields);
									$success = ($newID>0);
									if ($success ){
									
									print "Задача для обращения №".$ID." успешно создана<br/><br/>";
		...
		?>
		<input type="submit" value="Создать задачу" name="add_task">
	</td>
</tr>
